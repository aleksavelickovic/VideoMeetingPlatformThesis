        ...
        <div class="min-h-screen bg-page">
            ...
            @if (summary(); as call) {
                <main class="mx-auto w-full max-w-[632px] px-5 py-9"><h1 class="text-2xl font-semibold text-slate-900">
                    ...
                    <div class="mt-5 flex flex-wrap gap-3">@if (call.isHost && call.meeting?.recording?.presignedUrl) {
                        <a [href]="call.meeting?.recording?.presignedUrl" download class="btn-secondary">
                            <lucide-icon [img]="Download" class="size-4"/>
                            Download Recording</a>
                    }
                        @if (call.isHost) {
                            @for (participant of participants(); track participant.id) {
                                @if (participant.recordingPresignedUrl) {
                                    <a [href]="participant.recordingPresignedUrl" download class="btn-secondary">
                                        <lucide-icon [img]="Download" class="size-4"/>
...
export class PostCallPageComponent {
    ...
    protected readonly summary = this.state.callSummary;
    readonly participants = computed(() => this.summary()?.meeting?.participants ?? []);
    readonly duration = computed(() => formatDuration(this.summary()?.durationSeconds ?? 0));
    readonly joinedCount = computed(() => this.participants().filter(item => item.joinedAt).length);
