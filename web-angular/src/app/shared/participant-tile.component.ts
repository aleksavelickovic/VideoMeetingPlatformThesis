    ...
    template: `
        <article class="relative h-full min-h-0 overflow-hidden rounded-xl border bg-slate-100"
            ...
            <div class="absolute bottom-0 left-0 right-0 flex items-center justify-between bg-gradient-to-t from-black/80 px-3 pb-3 pt-8 text-xs text-white">
                <span class="truncate font-semibold">{{ isLocal() ? 'You' : participant().name || participant().identity }} @if (isHost) {
                    <small class="ml-1 text-amber">HOST</small>
                }</span>
                <span class="flex items-center gap-2">@if (screenSharing) {
                    <lucide-icon [img]="Monitor" class="size-4 text-brand"/>
                }
                    @if (muted) {
                        <lucide-icon [img]="MicOff" class="size-4 text-danger"/>
                    }</span>
            </div>
        </article>
    `
...
export class ParticipantTileComponent implements AfterViewChecked, OnDestroy {
    ...
    ngOnDestroy(): void {
        this.attached && this.attached.replaceChildren();
        this.attachedAudio && this.attachedAudio.replaceChildren()
    }
}
